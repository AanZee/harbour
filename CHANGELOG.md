...
- Change `/` to `math.div()` since using `/` for divisions is deprecated in SCSS.
...
- Add flex-shrink 0 to card\_\_media in card--horizontal
...
- Remove the layout\_\_section--spacing-100 modifier since this is the default layout section behavior.
...
- Add card**details border-bottom radius for projects to ensure the card border radius when the card**details has a background-color
...
- Clarify that layout\_\_inner is optional in the layout documentation
...
- Added layout**section--rounded-top and layout**section--rounded-bottom
